...
public:
	...
	relay_mode(asio::io_context &io_context_light, asio::io_context &io_context_heavy, KCP::KCPUpdater &kcp_updater_ref, task_thread_pool::task_thread_pool &parallel_pool, const user_settings &settings) :
	...
	relay_mode(relay_mode &&existing_relay) noexcept :
		io_context_light(existing_relay.io_context_light),
		io_context_heavy(existing_relay.io_context_heavy),
		...
		parallel_pool(existing_relay.parallel_pool),
